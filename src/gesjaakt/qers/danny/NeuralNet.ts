...
export class NeuralNet {
  ...
  public getWeight(l: number, j: number, k: number): number {
    return this.weightsAndBiases[
      this.offsets[l - 1] + j * (this.numNeuronsPerLayer[l - 1] + 1) + k + 1
    ];
  }

  public getBias(l: number, j: number): number {
    return this.weightsAndBiases[
      this.offsets[l - 1] + j * (this.numNeuronsPerLayer[l - 1] + 1)
    ];
  }
  ...
  public feedforward(inputs: number[]): number[] {
    if (this.weightsAndBiases.length === 0) {
      throw new Error(
        `Cannot feedforward before 'putWeights' or 'fromJSON' was called.`
      );
    }

    if (inputs.length !== this.numNeuronsPerLayer[0]) {
      throw new Error(
        `Expected ${this.numNeuronsPerLayer[0]} inputs, got ${inputs.length}.`
      );
    }
  ...
  private computeOffsets(): void {
    ...
    for (let l = 1; l < this.numNeuronsPerLayer.length - 1; l++) {
      this.offsets.push(
        (offset +=
          this.numNeuronsPerLayer[l - 1] * (this.numNeuronsPerLayer[l] + 1))
      );
    }
  }

  public putWeights(
    numNeuronsPerLayer: number[],
    weightsAndBiases: number[]
  ): void {
    ...
    if (n !== weightsAndBiases.length) {
      throw new Error(
        `The number of weights do not match the given architecture, expected ${n}, got ${weightsAndBiases.length}.`
      );
    }
